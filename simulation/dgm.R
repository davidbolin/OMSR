...
##        with S0 a Gaussian-kernel smoother (NOT L^{-alpha/2}).
## ---------------------------------------------------------------------
...
## ---------------------------------------------------------------------
## build_world(): everything that does NOT change across replicates 
## ---------------------------------------------------------------------
...
## i.e. U = tau^{-1} K^{-1} w with w ~ N(0, C0).  This is exactly the
## random effect whose precision the oracle estimator conditions on.
...
## ---------------------------------------------------------------------
## Scenario table.  beta0 = 1 is the target for every single-coefficient
## estimator; (beta_pw, beta_fc) is the truth for the two hybrid rows.
## ---------------------------------------------------------------------
SCENARIOS <- list(
...
## ---------------------------------------------------------------------
## simulate_dataset(): draw one replicate for a given scenario and n.
...
simulate_dataset <- function(world, scenario, n, sigma_e = 0.1, 
  ...
  y <- sc$beta_pw * Xobs + as.numeric(A %*% (signal + u)) +
       rnorm(n, sd = sigma_e)
