...
# Paired (Diebold--Mariano) difference in log-score vs the best model,
# with a spatial block-bootstrap SE.
perobs <- lapply(fits, function(f){
  g <- inla.group.cv(f, num.level.sets=M_LGOCV)
  if (is.null(g$cv)) return(NULL); -log(g$cv)
  })                 # per-observation log-score
...
nb <- 12L
bx <- cut(loc_km[,1], quantile(loc_km[,1], seq(0,1,length.out=nb+1)),
          include.lowest=TRUE, labels=FALSE)
by <- cut(loc_km[,2], quantile(loc_km[,2], seq(0,1,length.out=nb+1)),
          include.lowest=TRUE, labels=FALSE)
blk <- paste(bx, by)
byblk <- split(seq_len(n), blk)
ublk <- names(byblk)
B <- 2000L
bootse <- function(d){
  ms <- numeric(B)
...
write.csv(data.frame(source=rownames(r3f), r3f, row.names=NULL),
          "results/be_no2_threefield_ranges.csv", row.names=FALSE)
...
sp_rng <- function(nm){
  i <- grep(paste0("^Theta2 for ", nm, "$"), rownames(hp))
  if (length(i)){
    return(c(mean=sqrt(8*NU)/exp(hp[i,"mean"]), lo=sqrt(8*NU)/exp(hp[i,"0.975quant"]), hi=sqrt(8*NU)/exp(hp[i,"0.025quant"])))
  }  else{
    return(c(mean=NA,lo=NA,hi=NA) )
  }
}
spr <- rbind(`local (traffic+pop)`=sp_rng("field_loc"), industry=sp_rng("field_ind"))
cat("\n=== split two-field: ranges kappa^-1 (km) [mean, 95% CrI] ===\n"); print(round(spr,2))
write.csv(data.frame(field=rownames(spr), spr, row.names=NULL),
          "results/be_no2_split_ranges.csv", row.names=FALSE)

# credible intervals for the hybrid parameters
fh <- fits[["hybrid(bg+op)"]]
hp <- fh$summary.hyperpar
tr <- grep("^Theta[0-9]+ for field$", rownames(hp))
kap <- exp(hp[tr[2],"mean"])
kci <- exp(as.numeric(hp[tr[2],c("0.975quant","0.025quant")]))
fx <- fh$summary.fixed[, c("mean","0.025quant","0.975quant")]
pr <- rbind(setNames(data.frame(param=rownames(fx), fx), c("param","mean","lo","hi")),
            data.frame(param=c("beta_fc traffic","beta_fc industry","beta_fc population"),
                       mean=hp[tr[3:5],"mean"], lo=hp[tr[3:5],"0.025quant"],
                       hi=hp[tr[3:5],"0.975quant"]),
            data.frame(param="range kappa^-1 (km)",
                       mean=sqrt(8*NU)/kap, lo=sqrt(8*NU)/kci[1],
                       hi=sqrt(8*NU)/kci[2]))

cat("\n=== hybrid: posterior mean + 95% CrI ===\n"); print(pr, digits=4, row.names=FALSE)
write.csv(pr, "results/be_no2_inla_params.csv", row.names=FALSE)

# scale-free decomposition
bpw <- function(nm){
  if(nm %in% rownames(fh$summary.fixed)){
    return(as.numeric(fh$summary.fixed[nm,c("mean","0.025quant","0.975quant")]))
  }  else {
...
# Cameron--Martin correlations between the forcing channels
Rp <- attr(js, "R_p")
dimnames(Rp) <- list(js$coef, js$coef)
